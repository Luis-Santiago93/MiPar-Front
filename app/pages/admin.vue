<template>
  <main class="shell admin-page">
    ...
    <section v-if="tab === 'dashboard'" class="dashboard-section">
      <div class="admin-list-title"><div><h2>Resumen de ventas</h2><p class="help-text">Solo se incluyen pedidos marcados como vendidos.</p></div><button type="button" class="text-button" @click="reloadDashboard">Actualizar</button></div>
      <div class="metric-grid dashboard-metrics"><div class="metric-card"><span>Ventas concretadas</span><strong>{{ dashboard?.soldCount ?? 0 }}</strong></div><div class="metric-card"><span>Ingresos por ventas</span><strong>{{ money(dashboard?.soldTotal ?? 0) }}</strong></div><div class="metric-card"><span>Pares vendidos</span><strong>{{ dashboard?.soldPairs ?? 0 }}</strong></div><div class="metric-card"><span>Ticket promedio</span><strong>{{ money(dashboard?.averageTicket ?? 0) }}</strong></div><div class="metric-card"><span>Pedidos pendientes</span><strong>{{ dashboard?.pendingCount ?? 0 }}</strong></div><div class="metric-card"><span>Pares disponibles</span><strong>{{ dashboard?.availablePairs ?? 0 }}</strong></div></div>
      <p v-if="!dashboard?.salesByDay.length" class="admin-empty">Aún no hay ventas concretadas. Marca un pedido como vendido para generar estadísticas.</p>
      <template v-else>
        <div class="sales-chart-card"><div><h2>Ingresos por día</h2><p class="help-text">Comparación de los ingresos de cada fecha con ventas.</p></div><div class="sales-chart" role="img" aria-label="Gráfica de ingresos por día"><div v-for="day in dashboard.salesByDay" :key="day.date" class="sales-chart-column"><strong>{{ money(day.total) }}</strong><div class="sales-chart-track"><span :style="{ height: chartHeight(day.total) }"></span></div><small>{{ shortDate(day.date) }}</small></div></div></div>
        <div class="dashboard-breakdowns"><section class="dashboard-panel"><h2>Métodos de pago</h2><div v-for="payment in dashboard.payments" :key="payment.method" class="dashboard-ranking-row"><span><strong>{{ paymentLabel(payment.method) }}</strong><small>{{ payment.orders }} {{ payment.orders === 1 ? 'venta' : 'ventas' }}</small></span><strong>{{ money(payment.total) }}</strong></div></section><section class="dashboard-panel"><h2>Productos más vendidos</h2><div v-for="(product, index) in dashboard.topProducts" :key="product.productId" class="dashboard-ranking-row"><span><strong>{{ index + 1 }}. {{ product.name }}</strong><small>{{ product.pairs }} {{ product.pairs === 1 ? 'par' : 'pares' }}</small></span><strong>{{ money(product.total) }}</strong></div></section></div>
        <div class="admin-list-title sales-detail-title"><h2>Detalle por día</h2></div><div class="daily-sales"><div class="daily-sales-row daily-sales-head"><strong>Fecha</strong><strong>Ventas</strong><strong>Pares</strong><strong>Ticket promedio</strong><strong>Ingresos</strong></div><div v-for="day in [...dashboard.salesByDay].reverse()" :key="day.date" class="daily-sales-row"><strong>{{ formatDashboardDate(day.date) }}</strong><span>{{ day.orders }}</span><span>{{ day.pairs }}</span><span>{{ money(day.orders ? day.total / day.orders : 0) }}</span><strong>{{ money(day.total) }}</strong></div></div>
      </template>
    </section>
    ...
    <section v-if="tab === 'orders'" class="admin-orders"><div class="admin-list-title"><h2>Pedidos</h2><button type="button" class="text-button" @click="() => refreshOrders()">Actualizar</button></div><p v-if="!orders?.length" class="admin-empty">Todavía no hay pedidos. Confirma uno desde la tienda para verlo aquí.</p><article v-for="item in orders ?? []" :key="item.id" class="admin-order"><div><strong>{{ item.id }}</strong><small>{{ new Date(item.createdAt).toLocaleString('es-MX') }}</small></div><div><strong>{{ item.customer.name }}</strong><small>{{ item.customer.phone }} · {{ item.customer.address }}</small><small v-if="item.customer.notes">{{ item.customer.notes }}</small><a v-if="item.customer.location" class="admin-map-link" :href="`https://www.openstreetmap.org/?mlat=${item.customer.location.lat}&mlon=${item.customer.location.lon}#map=17/${item.customer.location.lat}/${item.customer.location.lon}`" target="_blank" rel="noopener noreferrer">Ver punto de entrega ↗</a><a v-if="item.status !== 'cancelado'" class="whatsapp-button" :href="orderWhatsAppUrl(item)" target="_blank" rel="noopener noreferrer">Enviar confirmación por WhatsApp ↗</a></div><div><strong>{{ money(item.total) }}</strong><small>{{ item.zoneId }} · {{ item.deliveryDate }} · {{ item.deliveryTime }}</small><small>Pago al entregar: {{ paymentLabel(item.paymentMethod) }}</small><small v-for="line in item.items" :key="`${line.productId}-${line.color}-${line.size}`">{{ line.quantity }}× {{ line.productId }} · {{ line.color }} · {{ line.size }}</small></div><label>Estado <strong v-if="item.status === 'vendido'" class="final-status sold-status">Vendido · venta concretada</strong><strong v-else-if="item.status === 'cancelado'" class="final-status cancelled-status">Cancelado · inventario devuelto</strong><template v-else><select :value="item.status" @change="changeStatus(item.id, ($event.target as HTMLSelectElement).value)"><option value="nuevo">Nuevo</option><option value="confirmado">Confirmado</option><option value="vendido">Vendido</option><option value="cancelado">Cancelado</option></select><button type="button" class="sell-button" @click="markSold(item.id)">Concretar venta</button></template></label></article><p v-if="message" class="admin-message" role="status">{{ message }}</p></section>
    <section v-if="tab === 'requests'" class="admin-orders"><div class="admin-list-title"><h2>Solicitudes de calzado</h2><button type="button" class="text-button" @click="() => refreshRequests()">Actualizar</button></div><p class="help-text">Son consultas de disponibilidad, no pedidos ni ventas. No descuentan existencias.</p><p v-if="!requests?.length" class="admin-empty">Todavía no hay solicitudes de modelos o tallas agotadas.</p><article v-for="item in requests ?? []" :key="item.id" class="admin-order request-admin-row"><div><strong>{{ item.id }}</strong><small>{{ new Date(item.createdAt).toLocaleString('es-MX') }}</small></div><div><strong>{{ item.productName }}</strong><small>{{ item.color }} · Talla {{ item.size }}</small><small>{{ item.customer.name }} · {{ item.customer.phone }}</small><small v-if="item.customer.notes">{{ item.customer.notes }}</small><a v-if="item.status !== 'cancelada'" class="whatsapp-button" :href="requestWhatsAppUrl(item)" target="_blank" rel="noopener noreferrer">Enviar confirmación por WhatsApp ↗</a></div><label>Seguimiento <strong v-if="item.status === 'cancelada'" class="final-status cancelled-status">Cancelada</strong><select v-else :value="item.status" @change="changeRequestStatus(item.id, ($event.target as HTMLSelectElement).value as ProductRequest['status'])"><option value="nueva">Nueva</option><option value="contactada">Contactada</option><option value="cerrada">Cerrada</option><option value="cancelada">Cancelada</option></select></label></article><p v-if="message" class="admin-message" role="status">{{ message }}</p></section>
...
type OrderLine = { productId: string; name: string; color: string; size: number; quantity: number; unitPrice: number; lineTotal: number }
type Order = { id: string; createdAt: string; paymentMethod: PaymentMethod; customer: { name: string; phone: string; address: string; notes: string; location?: { lat: number; lon: number } }; items: CartItem[]; lines: OrderLine[]; zoneId: string; zoneName: string; deliveryDate: string; deliveryTime: string; total: number; status: string }
type ProductRequest = { id: string; createdAt: string; productId: string; productName: string; color: string; size: number; customer: { name: string; phone: string; notes: string }; status: 'nueva' | 'contactada' | 'cerrada' | 'cancelada' }
type InventoryItem = { productId: string; productName: string; color: string; size: number; stock: number }
type Dashboard = { soldCount: number; soldTotal: number; soldPairs: number; averageTicket: number; pendingCount: number; availablePairs: number; salesByDay: { date: string; orders: number; pairs: number; total: number }[]; payments: { method: PaymentMethod; orders: number; total: number }[]; topProducts: { productId: string; name: string; pairs: number; total: number }[] }
...
const paymentLabel = (method: PaymentMethod) => ({ efectivo: 'Efectivo', transferencia: 'Transferencia', tarjeta: 'Tarjeta' }[method] ?? 'Por definir')
const whatsappPhone = (phone: string) => { const digits = phone.replace(/\D/g, ''); return digits.length === 10 ? `52${digits}` : digits }
const whatsappUrl = (phone: string, message: string) => `https://wa.me/${whatsappPhone(phone)}?text=${encodeURIComponent(message)}`
function orderWhatsAppUrl(order: Order) {
  const products = order.lines.map(line => `• ${line.quantity}× ${line.name} — ${line.color}, talla ${line.size}`).join('\n')
  const message = [`Hola ${order.customer.name}, confirmamos tu pedido MiPar *${order.id}*.`, '', '*Productos:*', products, '', `*Total:* ${money(order.total)}`, `*Entrega:* ${order.zoneName} · ${order.deliveryDate} · ${order.deliveryTime}`, `*Dirección:* ${order.customer.address}`, `*Pago al entregar:* ${paymentLabel(order.paymentMethod)}`, '', 'Por favor, confírmanos si los datos son correctos.'].join('\n')
  return whatsappUrl(order.customer.phone, message)
}
function requestWhatsAppUrl(request: ProductRequest) {
  const message = [`Hola ${request.customer.name}, recibimos tu solicitud MiPar *${request.id}*.`, '', `*Modelo:* ${request.productName}`, `*Color:* ${request.color}`, `*Talla:* ${request.size}`, '', 'Queremos confirmar que sigues interesado en este modelo. Responde a este mensaje para continuar con la atención.'].join('\n')
  return whatsappUrl(request.customer.phone, message)
}
const formatDashboardDate = (value: string) => new Intl.DateTimeFormat('es-MX', { day: 'numeric', month: 'short', year: 'numeric' }).format(new Date(`${value}T12:00:00`))
const shortDate = (value: string) => new Intl.DateTimeFormat('es-MX', { day: 'numeric', month: 'short' }).format(new Date(`${value}T12:00:00`))
...
const chartHeight = (total: number) => `${Math.max(8, Math.round((total / Math.max(...(dashboard.value?.salesByDay.map(day => day.total) ?? [1]), 1)) * 100))}%`
...
async function changeStatus(id: string, status: string) {
  try { await $fetch(`/api/admin/orders/${id}`, { method: 'PATCH', body: { status } }); await Promise.all([refreshOrders(), refreshDashboard(), refreshInventory(), refreshProducts()]); message.value = status === 'vendido' ? 'Venta concretada.' : status === 'cancelado' ? 'Pedido cancelado. Las existencias fueron devueltas al almacén.' : 'Estado actualizado.' }
