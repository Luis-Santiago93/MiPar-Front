  ...
  <main class="shell delivery-page">
    ...
    <div v-else class="delivery-grid">
      <div class="delivery-form">
        ...
        <label for="customer-notes">Indicaciones adicionales <span class="optional">(opcional)</span></label><textarea id="customer-notes" v-model.trim="customer.notes" rows="2" placeholder="Ej. fachada azul" /><fieldset class="payment-methods"><legend>¿Cómo pagarás al recibir tu pedido?</legend><small>El pago se realizará cuando hagamos la entrega.</small><div class="payment-options"><label class="payment-option"><input v-model="paymentMethod" type="radio" value="efectivo" required /> Efectivo</label><label class="payment-option"><input v-model="paymentMethod" type="radio" value="transferencia" required /> Transferencia</label><label class="payment-option"><input v-model="paymentMethod" type="radio" value="tarjeta" required /> Tarjeta</label></div></fieldset>
      </div>
...
type Order = Quote & { id: string; zoneName: string; deliveryDate: string; paymentMethod: PaymentMethod; customer: { name: string; phone: string; address: string; notes: string; location?: { lat: number; lon: number } }; lines: { productId: string; name: string; color: string; size: number; quantity: number; unitPrice: number; lineTotal: number }[] }
...
function validateCheckout() {
  ...
  if (!paymentMethod.value) return 'Selecciona cómo pagarás al recibir tu pedido.'
...
async function placeOrder() {
  ...
  try {
    await nextTick()
    const registeredOrder = await $fetch<Order>('/api/orders', { method: 'POST', body: { items: cart.value, zoneId: zoneId.value, deliveryDate: deliveryDate.value, deliveryTime: deliveryTime.value, paymentMethod: paymentMethod.value, customer: { ...customer, ...(deliveryPoint.value ? { location: deliveryPoint.value } : {}) } } })
