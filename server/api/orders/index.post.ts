...
import { quoteOrder, store, type Order, type PaymentMethod } from '../../data/store'

export default defineEventHandler(async event => {
  const body = await readBody<{ items: CartItem[]; zoneId: string; deliveryDate: string; deliveryTime: string; paymentMethod: PaymentMethod; customer: { name: string; phone: string; address: string; notes?: string; location?: { lat: number; lon: number } } }>(event)
  ...
  if (!['efectivo', 'transferencia', 'tarjeta'].includes(body.paymentMethod)) throw createError({ statusCode: 400, statusMessage: 'Selecciona un método de pago' })
  ...
  const order: Order = {
    ...
    items: structuredClone(body.items), lines, zoneId: body.zoneId, zoneName: store.zones.find(zone => zone.id === body.zoneId)!.name, deliveryDate: body.deliveryDate, deliveryTime: body.deliveryTime, paymentMethod: body.paymentMethod,
